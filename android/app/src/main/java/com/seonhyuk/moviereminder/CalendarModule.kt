package com.seonhyuk.moviereminder
...
class CalendarModule(reactContext: ReactApplicationContext): ReactContextBaseJavaModule(reactContext), PermissionListener {
    override fun getName() = "CalendarModule" // rn에서 가져올 이름
    ...
    private fun requestPermission(callback: (granted: Boolean) -> Unit) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            val status = reactApplicationContext.checkPermission(
            ...
            val status = reactApplicationContext.checkSelfPermission(Manifest.permission.WRITE_CALENDAR)
            if (status == PackageManager.PERMISSION_GRANTED) {
                callback.invoke(true)
                return
            }
            ...
            mCallbacks.put(mRequestCode, fun(grantResults) {
                val granted = grantResults != null && grantResults[0] == PackageManager.PERMISSION_GRANTED
